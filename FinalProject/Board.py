...
class ButtonMove(Button):
	...
	def rollDice(self):
		...
		while roll<=self.tempList:
			roll=random.randint(0,6)
			if roll==6:
				break
			print("roll "+str(roll))
		if roll==6 :
			if board.current_player_index == 1:	
				self.flipedOne +=1 
			if board.current_player_index == 2:	
				self.flipedTwo +=1 
			if board.current_player_index == 3:	
				self.flipedThree +=1 
			if board.current_player_index == 4:	
				self.flipedFour +=1 
		
		if board.current_player_index == 1:	
			self.player_positions[0] = (self.boardList(roll,self.flipedOne))
			row,col =self.boardList(roll,self.flipedOne)
			board.move_player(row,col,1)
		if board.current_player_index == 2:	
			self.player_positions[1] = (self.boardList(roll,self.flipedTwo))
			row,col=self.boardList(roll,self.flipedTwo)
			board.move_player(row,col,2)
		if board.current_player_index == 3:	
			self.boardList() 
		if board.current_player_index == 4:	
			self.boardList()
	...
	def boardList(self,roll,fliped):
		...
		if fliped == 0 :	  
			self.tempList = left_right_up[roll][1]
			return left_right_up[roll][0],self.tempList
		elif fliped == 1:
			self.tempList = down_top_right[roll][0]
			
			return self.tempList,down_top_right[roll][1]
		elif fliped == 3:
			self.tempList = right_left_down[roll][1]
		
			return right_left_down[roll][0],self.tempList
		elif fliped == 4:
			self.tempList = top_dow_right[roll][0]
			
			fliped == 0
			return self.tempList,top_dow_right[roll][1]
		
		print("roll "+str(roll))
		print("temp "+str(self.tempList))
